...
$email=$_SESSION['useremail'];
    ...
    $tpe=$_REQUEST['button_type'];
    
      if($coin=='0'){
          
          $gtu=mysqli_query($con,"select * from register where email='$emaill'"); 
          $gtufetch=mysqli_fetch_array($gtu);
          $btadd=$gtufetch['btcaddress'];
      }
      else{
          
          $gtu=mysqli_query($con,"select * from register where email='$emaill'"); 
          $gtufetch=mysqli_fetch_array($gtu);
          $btadd=$gtufetch['ethaddress'];
      }
    
      if($recbuy == '')
      {
          $recbuy = $_SESSION['btc_add'];
      }
      //echo $recbuy; die;
      
 mysqli_query($con,"INSERT INTO `buyer` ( `email`, `amount`, `address`,`code`,`type`,`user_id`,`payment_method`,`coin_type`) VALUES ( '$emaill', '$buy', '$btadd','$rcode','$type1','$user_id','$btcIn',$coin)");
 
$data=mysqli_query($con,"SELECT bank_detail.*, buyer.* from bank_detail LEFT JOIN buyer ON bank_detail.tid = buyer.code where buyer.type = 1 and buyer.amount = '$buy' and buyer.coin_type ='$coin' and buyer.email !='$emaill' ");
 
$data1=mysqli_query($con,"SELECT bank_detail.*, buyer.* from bank_detail LEFT JOIN buyer ON bank_detail.tid = buyer.code where buyer.type = 1 and buyer.amount = '$buy' and buyer.coin_type ='$coin' and buyer.email !='$emaill' ");
